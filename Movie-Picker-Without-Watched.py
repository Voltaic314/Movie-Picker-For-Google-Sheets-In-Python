...
watched_or_unwatched_prompt = str(input("Would you like to watch a new random movie? \nPlease type: Y for yes or X to stop the program:\n")).upper()

result_headers_y = sheet.values().get(spreadsheetId=config.config_stuff['SAMPLE_SPREADSHEET_ID'],
                            range="Movies!A2:F2").execute()

column_headers_y = result_headers_y.get('values', []) #get values from spreadsheet

running = True
while running:
    if watched_or_unwatched_prompt == "Y":

        #grab the sheet info from the API
        result_y= sheet.values().get(spreadsheetId=config.config_stuff['SAMPLE_SPREADSHEET_ID'],
                                    range="Movies!A:F").execute()

        #build that into a list of lists
        unwatched_movies = result_y.get('values', [])
        ...
        time.sleep(3)
        print(column_headers_y)# This prints this line into text to make the output easier to understand
